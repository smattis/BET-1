...
# reference QoI
Q_ref =  np.dot(np.array(ref_lam), Q_map)
...
if deterministic_discretize_D == True:
  (d_distr_prob, d_distr_samples, d_Tree) = simpleFunP.uniform_hyperrectangle(data=data_e,
                                              Q_ref=Q_ref, bin_ratio=0.3, center_pts_per_edge = 1)
else:
...
#                                                                     d_Tree=d_Tree)
(P, lam_vol, io_ptr) = calculateP.prob(samples, data_e, d_distr_prob, d_distr_samples)
