...
inds = [2]
samples = np.loadtxt("results/samples.txt")[0:10200,:]#[:,inds]
samples = samples - lam_domain[:,0]
samples = samples/(lam_domain[:,1]-lam_domain[:,0])
data = np.loadtxt("results/data.txt")[:][0:10200]
ee = np.loadtxt("results/ee.txt")[:][0:10200]
deriv = np.loadtxt("results/gradients1.txt")[0:10200,:]
...
(P, lam_vol, io_ptr) = calculateP.prob_exact(samples_all, data_all, d_distr_prob, d_distr_samples, lam_domain)

p_event = calculateEventP.prob_event(samples = samples_all,
                                     lam_domain = lam_domain,
                                     P = P,
...
if comm.rank == 0:
  print "Box probability", prob_box

me = calculateError.model_error(samples_all,
                                data_all,
                                error_estimate = error_estimate_all,
                                lam_vol = lam_vol_all,
                                rho_D_M = d_distr_prob,
                                rho_D_M_samples = d_distr_samples)
