...
def plot_2D_voronoi_ee(sample_set, colormap_type='BuGn', 
                       filename="file", 
                       lam_ref=None, interactive=False,
                       lambda_label=None, file_extension=".png"):
    ...
    # Check for global probabilities
    if sample_obj._error_id is None:
        if sample_obj._error_id_local is None:
            raise missing_attribute("Error ID")
        else:
            sample_obj.local_to_global()
    if sample_obj._values is None:
        sample_obj.local_to_global()
